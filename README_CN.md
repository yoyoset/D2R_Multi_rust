...
1. **准备**：Windows 10/11 x64；战网安装在默认路径 `C:\Program Files (x86)\Battle.net` 且勾选「为所有用户安装」；游戏/镜像所在分区为 NTFS（目录镜像依赖）。
2. **运行**：右键 `d2r-rust.exe` →「以管理员身份运行」。
3. **添加账号**：「账号管理」→「添加账号」，绑定（或新建）一个本地 Windows 用户、填好密码，并确认**基准路径**（战网里配置的游戏目录，可一键采用快照建议）——它是自动备份的比对依据。只做战网免登录切换、不玩 D2R 多开的账号，勾选「非 D2R 账户」即可免填。
4. **启动**：回仪表盘选中账号 →「启动游戏」，之后免登录（免登录凭据与该 Windows 用户的密码绑定，**别再改密码**，建议勾选「密码永不过期」）。
   > 💡 小建议：如果是**刚新建**的 Windows 用户，第一次登录战网最好自己手动切换过去登录一遍（Win+L →「切换用户」），而不是完全指望工具的跨用户启动一步到位——一个本地从没登录过的全新账号，走跨用户桥接首次登录偶尔会有点不确定，手动走一遍就完全绕开了这个问题。
5. **多开**：选另一个账号再点启动即可；实例锁由程序自动清理，快照在每次启动前自动轮巡备份。
