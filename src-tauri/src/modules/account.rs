use crate::modules::win32_safe::{process, mutex};
...
pub fn launch_game(app: &AppHandle, account: &Account, _game_path: &str) -> Result<u32, AccountError> {
    ...
    let mut config = crate::modules::config::AppConfig::load(app)
        .map_err(|_| AccountError::InvalidPath)?; 
    ...
    let working_dir = bnet_path_buf.parent()
        .map(|p| p.to_string_lossy().to_string());

    // Logic: Identify Current User vs Sandbox User
    let current_user = crate::modules::win_user::get_whoami().to_lowercase();
...
fn get_bnet_path() -> Option<PathBuf> {
    ...
    if let Ok(output) = Command::new("reg")
        .args(["query", r"HKLM\SOFTWARE\WOW6432Node\Blizzard Entertainment\Battle.net\Capabilities", "/v", "ApplicationIcon"])
        .output() {
                ...
                if let Some(user) = users.get_user_by_id(user_id) {
                    ...
                    for requested_user in &usernames {
                        ...
                        if proc_user == normalized_requested {
                            ...
                            if is_bnet { entry.bnet_active = true; }
                            if is_d2r { entry.d2r_active = true; }
                        }
                    }
                }
            }
