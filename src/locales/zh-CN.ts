export default {
    ...
    "setting_show_snapshot_reminder": "显示快照提醒横幅",
    "setting_show_snapshot_reminder_desc": "在仪表盘与账号管理顶部显示「最后启动的账号需手动保存快照」提醒。关闭横幅后可在此重新开启。",
    ...
    "snapshot_reminder_title": "重要：最后启动的账号需手动保存快照",
    "snapshot_reminder_body": "受多开机制限制，本工具与 D2R 没有直接联系、也不会过度检测游戏，因此自动备份只在「下次启动」时进行。这意味着你最后启动的那个账号不会被自动备份——玩完后请用账号上的「保存快照」按钮手动保存一次。提示：使用「序列启动」时，全部启动完成后，点序列迷你窗里的「完成并备份」按钮即可保存最后一个账号的快照。",
    "snapshot_reminder_seq": "序列全部启动完成后，请在序列迷你窗里点「完成并备份」按钮来保存最后一个账号的快照（建议先把战网/游戏设置调整好再点）。若改点 ✕ 关闭，则不会备份。",
    "snapshot_reminder_dismiss_hint": "输入 yes 关闭此提示：",
    "guide_fig_save_snapshot": "账号上的「保存快照」按钮（软盘图标）",
    "guide_fig_seq_finish": "序列完成后，点「完成并备份」保存最后一个账号",
    "snapshot_reminder_close": "关闭",
