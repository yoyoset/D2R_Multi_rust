export default {
    ...
    "seq_backup_content_mismatch": "Not backed up: the current Battle.net config does not match {{user}}'s baseline path/snapshot — it may have been rewritten by an out-of-band Battle.net, or you changed the path this session. If the current config is correct, use \"Save Snapshot\" on that account's card instead.",
    ...
    "baseline_path": "Baseline Path (as configured in Battle.net)",
    "baseline_path_hint": "The directory you selected when locating the game in the Battle.net client for this account; with mirror junctions, enter the mirror directory, not the real one. THE sole backup criterion: auto-backup (seamless rotation) only runs when it matches the live Battle.net config path. Left empty, this account is excluded from auto-backup.",
    "baseline_from_snapshot": "Read from snapshot",
    "baseline_no_snapshot_paths": "No game path found in this account's snapshot",
    "baseline_mismatch_title": "Baseline path differs from snapshot",
    "baseline_mismatch_desc": "Baseline you entered: {{baseline}}\nPath recorded in the snapshot: {{snapshot}}\n\nIf you moved the game / changed the mirror, choose \"Use mine\" (then confirm the path in Battle.net and re-save the snapshot). If it was a typo, \"Adopt snapshot value\" fixes it in one click.",
    "baseline_use_mine": "Use mine",
    "baseline_adopt_snapshot": "Adopt snapshot value",
    "baseline_mismatch_toast": "{{user}}'s Battle.net config/snapshot does not match its baseline path — if you moved the game, update the baseline in the account editor, or confirm the path in Battle.net and re-save the snapshot",
    "strict_baseline": "Sole baseline",
    "strict_baseline_hint": "When checked, this baseline is absolute: mismatches cancel the backup silently (log only), no prompts.",
    "baseline_required": "Please select a baseline path first (the directory chosen when locating the game in Battle.net)",
    "password_required": "Please enter this Windows user's password (only the current logged-in user may leave it empty)",
    "baseline_conflict_title": "Path mismatch: your ruling needed",
    "baseline_conflict_desc": "Account {{user}}'s current Battle.net config path: {{live}}\nIt differs from the baseline: {{baseline}}\n\nThe disputed config has been stashed. Your ruling:",
    "baseline_conflict_discard": "Cancel backup",
    "baseline_conflict_adopt": "Update baseline & back up",
    "baseline_conflict_multi_hint": "Multiple game paths detected — the new baseline cannot be determined automatically. Choose Cancel backup, then confirm the baseline manually in the account editor.",
    "baseline_conflict_resolved": "Updated {{user}}'s baseline to {{path}} and completed the backup",
    "baseline_conflict_failed": "Arbitration failed: {{error}}",
    "account_type_non_d2r": "Non-D2R account (Battle.net login-switching only)",
    "account_type_non_d2r_hint": "For users who only switch Battle.net accounts without re-login and don't multibox D2R. No baseline path required; backups run on ledger ownership alone.",
    "baseline_seed_title": "Upgrade: baseline paths synced automatically",
    "baseline_seed_synced": "This upgrade seeded baseline paths for {{count}} account(s) from their existing snapshots:",
    "baseline_seed_manual_needed": "The following accounts could not be determined automatically — set their baseline in the account editor (or mark them as non-D2R):",
    "baseline_seed_all_done": "All accounts have their baseline paths ready.",
    ...
    "logs": {
        "launcher": {
            ...
            "backing_up_owner": "Ledger + baseline verified — backing up previous account {{user}}'s snapshot (seamless rotation)",
            "backing_up_owner_plain": "Ledger ownership confirmed - backing up previous account {{user}}'s snapshot (non-D2R account)",
            "backup_skipped_no_baseline": "No baseline path set for {{user}} - auto-backup skipped. Confirm a baseline in the account editor to enable seamless rotation backups.",
            "backup_skipped_owner": "Skipped auto-backup for {{user}}: the live Battle.net config belongs to another account (path cross-contamination guard)",
            "backup_skipped_mismatch": "Skipped auto-backup for {{user}}: the live Battle.net config does not match this account's baseline path/snapshot (path cross-contamination guard)",
            "snapshot_baseline_mismatch": "Warning: the snapshot restored for {{user}} does not record its baseline path {{path}} — the snapshot may be stale or polluted; re-locate the game in Battle.net and save the snapshot again",
        ...
        "config": {
            "baseline_seeded": "Upgrade: seeded baseline paths for {{count}} account(s) from their existing snapshots",
