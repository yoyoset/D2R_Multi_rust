        ...
        <div className="flex flex-col p-4 mx-auto w-full h-full overflow-hidden">
            <div className="flex justify-between items-center mb-3 flex-shrink-0 px-2">
                <div className="flex flex-col gap-0">
                    <h2 className="text-[14px] md:text-[14px] font-black text-white uppercase tracking-tighter flex items-center gap-2">
                        <div className="w-1 h-3.5 bg-primary rounded-t-sm"></div>
                        {t('account_manager')}
                        <span className="ml-1 px-1 py-0.5 rounded-sm bg-zinc-900 border border-white/10 text-[10px] text-zinc-500 font-mono">
            ...
            <div className="flex-1 overflow-auto border border-white/5 bg-zinc-950/20 rounded-sm">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {accounts.map((account) => (
                            ...
                            >
                                ...
                                <td className="px-2 py-0.5 text-[14px] font-bold text-zinc-300 group-hover:text-white truncate flex items-center gap-1.5">
                                ...
                                <td className="px-2 py-0.5 truncate">
                                    {account.game_path ? (
                                        <span className="text-[10px] font-mono text-zinc-500 opacity-60 hover:opacity-100 transition-opacity whitespace-nowrap overflow-hidden text-ellipsis block" title={account.game_path}>
                                            {account.game_path}
                                        </span>
                                    ) : (
                                ...
                                <td className="px-2 py-0.5 text-[10px] text-primary/60 italic truncate">
                                    {account.note || ""}
                                </td>
                ...
                {accounts.length === 0 && (
                    <div className="text-center py-8 text-zinc-600 bg-zinc-900/5 text-[14px] font-mono uppercase tracking-widest">
                        {t('no_accounts_hint')}
                    </div>
                )}
            </div>
            ...
            <Modal isOpen={!!confirmDeleteId} onClose={() => setConfirmDeleteId(null)}>
                <ModalContent className="max-w-sm">
                    ...
                    <ModalBody className="p-4">
                        <p className="text-zinc-500 text-[10px] leading-normal font-mono uppercase italic">
