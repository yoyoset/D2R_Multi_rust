        ...
        <div className={cn("shrink-0 border-t border-white/5 bg-zinc-950/80 backdrop-blur-md overflow-hidden", className)}>
            ...
            >
                <div className="flex items-center gap-2">
                    {isExpanded ? <ChevronDown size={10} className="text-zinc-600" /> : <ChevronUp size={10} className="text-zinc-600" />}
                    <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500">{t('atomic_logs')}</span>
                </div>
                <button
                    onClick={(e) => {
                        e.stopPropagation();
            ...
            <div className={cn(
                "overflow-y-auto font-mono text-[9px] space-y-0.5 scrollbar-thin scrollbar-thumb-zinc-800 transition-all ease-in-out",
                ...
                {logs.length === 0 && (
                    <div className="flex items-center justify-center p-3 text-zinc-800 text-[9px] font-black uppercase tracking-[0.2em] italic">
                        {t('waiting_ops')}
                    </div>
                )}
            </div>
