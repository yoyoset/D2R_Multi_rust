    ...
    return (
        <Modal isOpen={isOpen} onClose={() => onClose(dontShowAgain)}>
            <ModalContent className="max-w-2xl">
                ...
                <ModalBody className="space-y-4 p-5 text-text-dim">
                    ...
                    <div className="bg-danger-500/5 border border-danger-500/10 rounded-lg p-5 space-y-3">
                        ...
                        <div className="space-y-2.5 pt-1">
                            <div className="flex gap-2.5 text-[13px]">
                                ...
                                <span className="text-text-dim">{t('guide_bnet_path_desc')} <code className="bg-black/40 px-1.5 py-0.5 rounded text-danger-300 font-mono text-[10px]">C:\Program Files (x86)\Battle.net</code></span>
                            </div>
                            <div className="flex gap-2.5 text-[13px]">
                                <span className="text-danger-300/80 font-bold whitespace-nowrap">{t('bnet_all_users')}：</span>
                                <span className="text-text-dim">{t('guide_bnet_install_all_users_desc')}</span>
                            </div>
                            <div className="flex gap-2.5 text-[13px]">
                                <span className="text-danger-300/80 font-bold whitespace-nowrap">{t('no_custom_path')}：</span>
                                <span className="text-text-dim">{t('guide_bnet_path_reason_desc')}</span>
                            </div>
                        </div>
                    </div>

                    {/* Game Path Section */}
                    <div className="bg-player-500/5 border border-player/10 rounded-lg p-5 space-y-2">
                    ...
                    {/* Launch Mode Section */}
                    <div className="bg-net-500/5 border border-net/10 rounded-lg p-5 space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <p className="text-[13px] text-text-dim leading-relaxed font-light">{t('guide_launch_managed_desc')}</p>
                            </div>
                            <div className="space-y-1.5">
                ...
                <ModalFooter className="flex justify-between items-center p-5">
                    ...
                    >
                        ...
                        <span className="text-[10px] text-text-dim group-hover:text-text-dim transition-colors">
                            {t('dont_show_again')}
                        </span>
                    </div>
