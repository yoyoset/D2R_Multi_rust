...
export const ModalFooterActions = ({ onClose, handleSave, isSaving, isEdit, isCreatingNew }: ModalFooterActionsProps) => {
    ...
    return (
        <ModalFooter className="flex-row items-center justify-end gap-3 px-6 pb-4 pt-3 border-t border-white/5 bg-bg/40 relative overflow-hidden group/footer">
            {/* Background Polish */}
            ...
            <Button
                variant="outline"
                className="text-text-dim h-9 px-5 border-white/5 hover:bg-surface-2 hover:text-text-dim rounded-sm text-[10px] font-black uppercase tracking-widest transition-all"
                onClick={onClose}
                disabled={isSaving}
            >
                {t('cancel')}
            ...
            <Button
                ...
                className={cn(
                    ...
                    isCreatingNew 
                        ? "bg-player/10 border-player/50 text-player hover:bg-player/20 shadow-[0_0_15px_rgba(16,185,129,0.2)]"
                        : "bg-net/10 border-net/50 text-net hover:bg-net/20 shadow-[0_0_15px_rgba(59,130,246,0.2)]"
                )}
            >
